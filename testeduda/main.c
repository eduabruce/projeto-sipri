...
struct Produto {
    ...
    int ativo; // <--- Campo para marcação lógica (1=ativo, 0=excluído)
};
...
int salvarConfig();
int carregarConfig();
int salvarProdutos(struct Produto produtos[], int qtd);
...
void pausar() {
    ...
    // Limpa o buffer de entrada antes de ler a nova linha
...
int salvarConfig() {
    FILE *f = fopen(ARQ_CONFIG, "wb");
    if (!f) {
        imprimir_erro("Nao foi possivel abrir o arquivo de configuracao para escrita.");
        return 0;
    }
...
int salvarProdutos(struct Produto produtos[], int qtd) {
    ...
    if (!f) {
        printf("Erro: não foi possível abrir o arquivo de produtos para escrita.\n");
        return 0;
    }

    // Escrita sequencial (reescrevendo todos os registros ativos na memória)
    for (int i = 0; i < qtd; i++) {
        if (fwrite(&produtos[i], sizeof(struct Produto), 1, f) != 1) {
            printf("Erro: falha ao salvar dados no arquivo.\n");
            fclose(f);
            return 0;
        }
    }
...
void configurarDespesasFixas() {
   ...
   if (buf[0] == 's' || buf[0] == 'S') {
    ...
    config.producao_mensal_unidades =
    lerIntValidado("Producao mensal (unidades/mes)", config.producao_mensal_unidades);
}
        if(!salvarConfig()){
...
void excluirProdutoIndex(struct Produto produtos[], int *qtd, int idx) {
    // Exclui fisicamente da memória (compactação)
...
void excluirProduto(struct Produto produtos[], int *qtd) {
    ...
    if (buf[0] == 's' || buf[0] == 'S') {
        ...
        // A exclusão física no array na RAM exige reescrever o arquivo para refletir a mudança no disco.
        salvarProdutos(produtos, *qtd);
    } else {
...
void editarProduto(struct Produto produtos[], int qtd) {
    ...
    limpar_tela();
    printf("%sEDITANDO: %s%s (ID: %d)\n", BOLD, p->nome, RESET, id);
    imprimir_linha('-', 50);
    ...
    // Passo 1: Recalcular e atualizar a memória
    calcularTudo(p);

    // Passo 2: Acesso Direto para salvar a mudança no disco

    FILE *f = fopen(ARQ_PRODUTOS, "r+b"); // Abrir para leitura e escrita
    if (f == NULL) {
        imprimir_erro("Erro ao abrir arquivo para edicao de acesso direto.");
        return;
    }

    // Calcular a posição de deslocamento
    long offset = (long)idx * sizeof(struct Produto);

    // Mover o ponteiro para o início do registro (Acesso Direto com fseek)
    if (fseek(f, offset, SEEK_SET) != 0) {
        imprimir_erro("Erro ao localizar registro para edicao.");
        fclose(f);
        return;
    }

    // Escrever a estrutura atualizada, SOBRESCREVENDO o registro antigo
    if (fwrite(p, sizeof(struct Produto), 1, f) != 1) {
        imprimir_erro("Falha ao salvar edicao no arquivo.");
    } else {
        imprimir_sucesso("Edicao salva!");
    }
...
void menuPosCadastro(struct Produto produtos[], int *qtd, int idxRecente) {
    ...
    while (1) {
        ...
        if (opc == 1) {
            if (salvarProdutos(produtos, *qtd))
...
void menuGerenciarProdutos(struct Produto produtos[], int *qtd) {
    ...
    do {
        ...
        imprimir_secao("GERENCIAR PRODUTOS");
        printf("%s1%s - Cadastrar novo produto\n", GREEN, RESET);
...
void cadastrarProduto(struct Produto produtos[], int *qtd) {
    ...
    struct Produto p;
    memset(&p, 0, sizeof(p));
    p.ativo = 1; // Marca como ativo ao cadastrar!
    do {
